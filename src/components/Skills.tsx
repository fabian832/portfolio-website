      ...
      <div className="max-w-6xl mx-auto"> {/* Increased max-width for better 3-column spacing */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {skills.map((s, i) => (
            <div 
              key={i} 
              className="p-5 bg-slate-50 dark:bg-slate-900/40 border border-slate-200 dark:border-slate-800/50 rounded-2xl group transition-all hover:shadow-md dark:hover:shadow-sky-500/5"
            >
              <div className="flex justify-between mb-3">
                <span className="font-bold text-sm md:text-base text-slate-800 dark:text-slate-200 group-hover:text-sky-500 transition-colors">
                  {s.name}
                </span>
                <span className="text-slate-400 font-mono text-xs md:text-sm">{s.level}%</span>
              </div>
